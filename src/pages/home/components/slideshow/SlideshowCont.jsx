...
export default function SlideshowCont({title,description,imgTitle,id}){
    ...
    return(
        <div style={{backgroundImage:`url(/public/images/${imgTitle}.jpg)`, left:id===slideshow.length?`-100vw`:`${(id-1)*window.innerWidth}px`, zIndex:id===slideshow.length?-50:50}} className={id===1? (className+" active" ): className}>
            <div className="flex justify-center items-center bg-gradient-to-r from-white to-transparent w-full h-full">
                ...
                <div className="title text-[27px] leading-[2rem] max-md:leading-[1.5rem] italic">
                     <span className="text-deeppink">{title.substring(0,title.indexOf(" "))}</span><span>{title.substring(title.indexOf(" "))}</span>
                </div>
                <div className="description text-[20px] mt-[20px] italic max-md:leading-[1.5rem]">
                     {description}
                </div>
